...
from mt_bots.cogs.play_cog import PlayCog


class MediaPlayer(commands.Bot):
    ...
    async def setup_hook(self):
        await self.add_cog(PlayCog(self))
        
    async def on_ready(self):
        self.logger.info(f'MediaPlayer object logged in as {self.user.name} (ID: {self.user.id})')
